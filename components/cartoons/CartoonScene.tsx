...
export default function CartoonScene({
  ...
  return (
    <div
      className={`rounded-2xl p-6 shadow-xl ${backgrounds[background]} ${className} ${mounted ? 'animate-[fadeInUp_0.5s_ease-out]' : 'opacity-0'
        }`}
      dir="rtl"
    >
      <div className="flex flex-col items-center text-center space-y-6">
        ...
        <div className="flex items-center justify-center gap-8">
          ...
          {character === 'wise-guide' || character === 'both' ? (
            <WiseGuide
              emotion={emotion === 'celebrating' ? 'happy' : emotion as any}
              size="large"
              animation={
                emotion === 'explaining' || emotion === 'thinking' ? 'speak' :
                  'idle'
              }
            />
          ) : null}
        ...
        {/* Message bubble */}
        <div
          className={`bg-white rounded-2xl p-6 shadow-lg border-2 border-green-300 max-w-md relative ${mounted ? 'animate-[bubbleIn_0.4s_ease-out_0.3s_both]' : 'opacity-0 scale-90'
            }`}
        >
          {/* Speech bubble tail */}
        ...
        {showButton && onContinue && (
          <button
            className={`bg-gradient-to-r from-green-500 to-green-600 text-white px-8 py-3 rounded-xl font-bold text-lg shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95 ${mounted ? 'animate-[buttonIn_0.3s_ease-out_0.6s_both]' : 'opacity-0 scale-90'
              }`}
            onClick={onContinue}
          >
            {buttonText} ←
