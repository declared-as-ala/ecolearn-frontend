...
const apiRequest = async (
  ...
  // This prevents unnecessary cache busting and reduces Turbopack rebuilds
  let url = `${API_URL}${endpoint}`;
  const needsFreshData = endpoint.includes('/notifications') || 
                         endpoint.includes('/progress') ||
                         endpoint.includes('/me');
  
  if ((options.method === 'GET' || !options.method) && needsFreshData && !endpoint.includes('_t=')) {
  ...
  // Disable response logging to prevent Turbopack issues
  // if (process.env.NODE_ENV === 'development') {
...
export const coursesAPI = {
  ...
  watchVideo: async (courseId: string, timeSpent?: number, progressPercent?: number): Promise<any> => {
    const response = await apiRequest(`/courses/${courseId}/video/watch`, {
      method: 'POST',
      body: JSON.stringify({ timeSpent, progressPercent }),
    });
    const result = await response.json();
