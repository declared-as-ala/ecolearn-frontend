...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Only redirect if auth is done and user exists
    if (authLoading) {
      return; // Still loading, wait
    }

    if (user && !hasRedirectedRef.current && typeof window !== 'undefined') {
      // Check if we're actually on the login page to avoid redirecting from other pages
      const currentPath = window.location.pathname;
      if (currentPath === '/login' || currentPath === '/') {
        hasRedirectedRef.current = true;
        
        // For students, check if they have selected a grade level
  ...
  }, [authLoading, user?.id]); // Only depend on authLoading and user.id
