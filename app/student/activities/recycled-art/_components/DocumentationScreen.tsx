...
export default function DocumentationScreen({ onNext, onUpdate, data }: DocumentationScreenProps) {
    ...
    const [capturedPhoto, setCapturedPhoto] = useState<string | null>(data.modelImage || null);
    ...
    const capturePhoto = () => {
        if (videoRef.current && canvasRef.current) {
            ...
            if (context) {
                ...
                setPhotoTaken(true);
                stopCamera();
                onUpdate({ modelImage: photoDataUrl });
            }
        }
    };
    ...
    const canProceed = photoTaken && selectedEmoji && note;
                    ...
                    <div className="bg-gray-200 rounded-2xl p-4 text-center min-h-[250px] flex items-center justify-center relative overflow-hidden">
                        {!photoTaken ? (
                            ...
                            >
                                ...
                                <div className="absolute top-4 left-4 bg-purple-500 text-white px-4 py-2 rounded-full font-bold">
                                    ✅ تم التقاط الصورة
                                </div>
                            </motion.div>
                        )}
