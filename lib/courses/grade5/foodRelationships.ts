...
export const foodRelationshipsGrade5: Course = {
    ...
    games: [
        ...
        // New High-Fidelity Race Mode
        {
            id: 'g1',
            type: 'dragdrop',
            title: 'سباق السلسلة الغذائية',
            description: 'أكمل السلسلة الغذائية! اختر الغذاء الصحيح لكل حيوان ليتحرك في السباق 🏁🌿',
            points: 40,
            gameData: {
                targets: [
                    { id: 't_plant', label: 'نبات/عشب', emoji: '🌿' },
                    { id: 't_insect', label: 'حشرة', emoji: '🦗' },
                    { id: 't_flesh', label: 'لحم (فريسة)', emoji: '🥩' },
                    { id: 't_seed', label: 'حبوب', emoji: '🌾' },
                ],
                lanes: [
                    {
                        id: 'l1',
                        label: 'المسار 1: آكل النبات',
                        racer: { id: 'r1', label: 'أرنب 🐰', emoji: '🐰', correctTargetId: 't_plant' }
                    },
                    {
                        id: 'l2',
                        label: 'المسار 2: آكل الحشرات',
                        racer: { id: 'r2', label: 'ضفدع 🐸', emoji: '🐸', correctTargetId: 't_insect' }
                    },
                    {
                        id: 'l3',
                        label: 'المسار 3: آكل اللحوم',
                        racer: { id: 'r3', label: 'أسد 🦁', emoji: '🦁', correctTargetId: 't_flesh' }
                    },
                    {
                        id: 'l4',
                        label: 'المسار 4: آكل الحبوب',
                        racer: { id: 'r4', label: 'عصفور 🐦', emoji: '🐦', correctTargetId: 't_seed' }
                    },
                ],
        ...
        // 🎮 GAME 2: "مهمة إنقاذ البيئة" (Eco Rescue Mission)
        // Detailed Problem-Solution Mode
        {
            id: 'g2',
            type: 'rescue',
            title: 'مهمة إنقاذ البيئة',
            description: 'كن بطلاً للبيئة! عالج المشاكل بالأدوات الصحيحة 🧹🌱🛡️',
            points: 40,
            gameData: {
                items: [
                    {
                        id: 'i1', type: 'waste', label: 'نفايات',
                        x: 20, y: 70, icon: '🗑️',
                        correctAction: 'clean', successIcon: '✨'
                    },
                    {
                        id: 'i2', type: 'plant_cut', label: 'شجرة مقطوعة',
                        x: 50, y: 50, icon: '🪓',
                        correctAction: 'plant', successIcon: '🌳'
                    },
                    {
                        id: 'i3', type: 'animal_hurt', label: 'طائر مصاب',
                        x: 80, y: 60, icon: '🤕',
                        correctAction: 'protect', successIcon: '🐦'
                    },
                    {
                        id: 'i4', type: 'waste_water', label: 'تلوث الماء',
                        x: 30, y: 80, icon: '🛢️',
                        correctAction: 'clean', successIcon: '💧'
                    },
                    {
                        id: 'i5', type: 'plant_withered', label: 'زهرة ذابلة',
                        x: 70, y: 80, icon: '🥀',
                        correctAction: 'plant', successIcon: '🌻'
                    },
                ],
        ...
        // 🎮 GAME 3: "بناء الشبكة الغذائية" (Creative Food Web)
        // Strict Graph Mode
        {
            id: 'g3',
            type: 'construction',
            title: 'بناء الشبكة الغذائية',
            description: 'ابنِ شبكة الحياة! صل الكائنات ببعضها (من الأكل إلى المأكول) 🦉🌍',
            points: 45,
            gameData: {
                minConnections: 6,
                nodes: [
                    // Row 1: Producers/Sources
                    { id: 'n_sun', label: 'شمس', type: 'source', icon: '☀️', x: 50, y: 15 },
                    { id: 'n_grass', label: 'عشب', type: 'producer', icon: '🌿', x: 20, y: 40 },
                    { id: 'n_flower', label: 'زهرة', type: 'producer', icon: '🌸', x: 80, y: 40 },

                    // Row 2: Primary Consumers
                    { id: 'n_rabbit', label: 'أرنب', type: 'consumer', icon: '🐇', x: 30, y: 65 },
                    { id: 'n_bee', label: 'نحلة', type: 'consumer', icon: '🐝', x: 70, y: 65 },

                    // Row 3: Secondary Consumers
                    { id: 'n_fox', label: 'ثعلب', type: 'consumer', icon: '🦊', x: 50, y: 85 },
                ],
                // STRICT Valid Connections (Direction: Source -> Target / Eaten -> Eater)
                validConnections: [
                    { from: 'n_sun', to: 'n_grass' },
                    { from: 'n_sun', to: 'n_flower' },
                    { from: 'n_grass', to: 'n_rabbit' },
                    { from: 'n_flower', to: 'n_bee' },
                    { from: 'n_rabbit', to: 'n_fox' },
                    { from: 'n_bee', to: 'n_flower' }, // Bee pollinates flower? But game is "who eats who". Bee eats nectar from flower.
                    // Let's assume connection flow: Eaten -> Eater.
                    // Flower -> Bee.
                ],
