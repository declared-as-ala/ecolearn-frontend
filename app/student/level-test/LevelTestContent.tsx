...
import { CheckCircle, ShieldAlert, Trophy, Video } from 'lucide-react';
...
export default function LevelTestContent() {
    ...
    const [showPostTestVideo, setShowPostTestVideo] = useState(false);
    ...
    const randomizeQuestions = (qs: Question[]): Question[] => {
        return qs.map(q => ({
            ...q,
    ...
    useEffect(() => {
        ...
        // Randomize question options before setting
        const baseQuestions = level === '5eme' ? questions5 : questions6;
        const randomizedQuestions = randomizeQuestions(baseQuestions);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setResult(result);
            setShowPostTestVideo(true); // Show post-test video
    ...
    // Redirect to post-test video page after successful completion
    useEffect(() => {
        if (testCompleted && result && result.completed && normalizedLevel) {
            // Check if already watched post-test video
            const watched = typeof window !== 'undefined' 
                ? localStorage.getItem(`postTestVideoSeen_${normalizedLevel}`) === 'true'
                : false;
            
            if (watched) {
                // Already watched, go directly to courses
                router.replace('/student/courses');
            } else {
                // Redirect to post-test video page
                router.replace(`/student/post-test-video?level=${normalizedLevel}`);
            }
        }
    }, [testCompleted, result, normalizedLevel, router]);
    ...
    // Post-Test Video Screen
    if (testCompleted && result && showPostTestVideo) {
        const postTestVideoUrl = normalizedLevel === '5eme' ? '/videos/test-5eme.mp4' : '/videos/test-6eme.mp4';
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-purple-50 flex items-center justify-center p-4" dir="rtl">
                <Card className="w-full max-w-4xl border-4 border-blue-200 shadow-2xl rounded-3xl overflow-hidden">
                    <CardHeader className="bg-gradient-to-r from-blue-100 to-green-100 border-b-2 border-blue-200 text-center">
                        <Trophy className="w-16 h-16 text-yellow-500 mx-auto mb-2" />
                        <CardTitle className="text-3xl text-gray-800">🎉 مبروك! اجتزت الاختبار</CardTitle>
                        <p className="text-blue-700 font-bold mt-2">شاهد الفيديو التوضيحي قبل البدء في الدروس</p>
                    </CardHeader>
                    <CardContent className="p-6 space-y-6">
                        {/* Video Player */}
                        <div className="bg-black rounded-2xl overflow-hidden">
                            <video
                                key={postTestVideoUrl}
                                controls
                                preload="metadata"
                                playsInline
                                className="w-full h-auto"
                                onError={() => {
                                    console.error('❌ Failed to load post-test video:', postTestVideoUrl);
                                }}
                            >
                                <source src={postTestVideoUrl} type="video/mp4" />
                        ...
                        <div className="bg-gradient-to-r from-green-50 to-blue-50 border-2 border-green-200 rounded-2xl p-6">
                            <div className="grid grid-cols-3 gap-4 text-center">
                                <div>
                                    ...
                                    <p className="text-xl font-bold text-gray-800">{result.level === '5eme' ? 'السنة الخامسة' : 'السنة السادسة'}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-600">النتيجة</p>
                                    <p className="text-3xl font-extrabold text-green-700">{result.score}/{result.level === '5eme' ? 10 : 12}</p>
                                </div>
                                <div>
                        ...
                        <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-200 rounded-2xl p-4 text-center">
                            <p className="text-lg font-bold text-purple-800 mb-2">🌟 أنت الآن جاهز للبدء في مغامرتك البيئية!</p>
                            <p className="text-sm text-purple-700">انقر على الزر أدناه للانتقال إلى الدورات</p>
                        </div>

                        {/* Action Buttons */}
                        <div className="flex justify-center gap-3">
                            <Button 
                                className="rounded-2xl px-8 py-4 font-bold text-lg bg-gradient-to-r from-green-500 to-blue-500 hover:from-green-600 hover:to-blue-600 text-white shadow-lg transform hover:scale-105 transition-all" 
                                onClick={() => router.replace('/student/courses')}
                            >
                                الانتقال إلى الدورات الآن 🚀📚
                            </Button>
                        </div>
                    </CardContent>
                </Card>
    ...
    if (testCompleted && result) {
        // Success Screen (old, fallback if no video)
        return (
                ...
                <Card className="w-full max-w-3xl border-4 border-green-200 shadow-2xl rounded-3xl overflow-hidden">
                    <CardHeader className="bg-gradient-to-r from-green-100 to-amber-100 border-b-2 border-green-200">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <CardTitle className="text-2xl text-gray-800">✅ أحسنت! اجتزت الاختبار التشخيصي</CardTitle>
                                <p className="text-sm text-green-700 font-bold">انقر على الزر أدناه للانتقال إلى الدورات 📚</p>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent className="p-6 space-y-6">
                        ...
                        <div className="bg-gradient-to-r from-blue-50 to-green-50 border-2 border-blue-200 rounded-2xl p-4 text-center">
                            <p className="text-lg font-bold text-blue-800 mb-2">🎉 ممتاز! أنت الآن جاهز للبدء!</p>
                            <p className="text-sm text-blue-700">انقر على الزر أدناه للانتقال مباشرة إلى الدورات، أو انتظر 5 ثوانٍ للانتقال التلقائي</p>
                        </div>

                        <div className="flex justify-center gap-3">
                            <Button 
                                variant="outline" 
                                className="rounded-2xl px-6 py-3 font-bold border-amber-400 text-amber-700 hover:bg-amber-50" 
                                onClick={handleRetry}
                            >
                                إعادة الاختبار 🔄
                            </Button>
                            <Button 
                                className="rounded-2xl px-8 py-4 font-bold text-lg bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white shadow-lg transform hover:scale-105 transition-all" 
                                onClick={() => router.replace('/student/courses')}
                            >
                                الذهاب إلى الدورات الآن 🚀📚
                            </Button>
                        </div>
                    </CardContent>
                </Card>
