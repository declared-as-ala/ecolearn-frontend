...
export default function CourseDetailPage() {
  ...
  // Initialize courseId ref only once
  if (!courseIdRef.current && courseId) {
    courseIdRef.current = courseId;
  }
  
  // Use ref value for stability
  const stableCourseId = courseIdRef.current || courseId;
  
  const hasLoadedCourseRef = React.useRef(false);
  const currentCourseIdRef = React.useRef<string | null>(null);
  const hasRedirectedRef = React.useRef(false);
  const isLoadingRef = React.useRef(false);
  const routerRef = React.useRef(router);
  routerRef.current = router;

  // Single useEffect that runs only once per courseId
  useEffect(() => {
    const currentId = stableCourseId;
    
    if (!currentId) {
      return;
    }

    // Early return if already loaded for this course
    if (hasLoadedCourseRef.current && currentCourseIdRef.current === currentId) {
      return;
    }
    ...
    const loadCourse = async () => {
      ...
      // Check user from localStorage first (don't wait for AuthContext)
      ...
      const gradeLevel = parsedUser.gradeLevel || (typeof window !== 'undefined' ? parseInt(localStorage.getItem('gradeLevel') || '0') : 0);
      if (!gradeLevel || (gradeLevel !== 5 && gradeLevel !== 6)) {
        isLoadingRef.current = false;
        setLoading(false);
        if (!hasRedirectedRef.current) {
          hasRedirectedRef.current = true;
          routerRef.current.replace('/student/select-level');
        }
        return;
      }

      // Enforce diagnostic test completion (check once only)
      if (!hasLoadedCourseRef.current) {
        try {
          ...
          if (!testStatus.completed) {
            ...
            if (!hasRedirectedRef.current) {
              hasRedirectedRef.current = true;
              routerRef.current.replace(`/student/level-test?level=${levelKey}`);
            }
            return;
          }
        } catch (e) {
          // Silent fail - allow access if test status check fails
        }
      }

      // Find local course data
      const localData = coursesData.find(c => c.id === currentId);
      if (!localData) {
        isLoadingRef.current = false;
        setLoading(false);
        ...
        if (!hasRedirectedRef.current) {
          hasRedirectedRef.current = true;
          routerRef.current.replace('/student/courses');
        }
        return;
      }
      ...
      try {
        const data = await coursesAPI.getOne(currentId);
        setApiCourse(data);
        setVideoWatched(data.progress?.videoWatched || false);
      } catch (error: unknown) {
        // Silent fail - fallback to local data only
      } finally {
        setLoading(false);
        isLoadingRef.current = false;
        hasLoadedCourseRef.current = true;
        currentCourseIdRef.current = currentId;
      }
    };

    // Reset flags when courseId changes
    if (currentCourseIdRef.current !== currentId && currentCourseIdRef.current !== null) {
  ...
  }, [stableCourseId]); // Only depend on stable courseId
  ...
  const handleExerciseComplete = async (exerciseId: string, score: number, maxScore: number) => {
    try {
      ...
      // Don't update user context here to prevent infinite loops
      // User points/badges will be updated on next page load
  ...
  const handleVideoProgressUpdate = async (progressPercent: number, timeSpent?: number) => {
    try {
      await coursesAPI.watchVideo(courseId, timeSpent || 0, progressPercent);
      // Update local state
  ...
  const handleVideoEnd = async () => {
    try {
      try {
        // Mark as 100% watched
        await coursesAPI.watchVideo(courseId, 0, 100);
      } catch (err: any) {
        if (err.message?.includes('404') || err.message?.includes('not found')) {
      ...
      // Don't refresh user here to prevent loops
      // Points will be visible on next navigation
      // if (refreshUser) {
      //   await refreshUser();
      // }
    } catch (error: any) {
...
دعنا نبدأ رحلتنا الممتعة لنتعرف على كيفية حماية كوكبنا! 💚`}
  ...
  // Priority: course.videoUrl (real MP4) > apiCourse > videos array
  const videoUrl = course.videoUrl || apiCourse?.sections?.video?.url || apiCourse?.videoUrl || '';
  ...
  const hasVideos = !!videoUrl || hasAnimatedVideo || hasStoryboard || (videos && videos.length > 0);
      ...
      <main className="mr-20 lg:mr-64 p-6 max-w-6xl mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
          ...
          <TabsContent value="video" className="mt-6">
            {/* Priority: Real MP4 Video > AnimatedVideo > Storyboard */}
            {videoUrl ? (
